...
| Guardrails HTTP wrapper (`:8088`) | Working. PII masking, injection preflight, topical rail, and 9 output rails (verdict / CoT / citation / absolutism / stale-attestation / jurisdiction / currency / standards-version / CVE). |
...
5. **Output-rail matrix expansion.** Nine rails land in v0.6.0. Candidate next rails already scoped: non-existent regulator names (EDPB / ENISA confabulations), fabricated case law and enforcement-action IDs, fabricated incident-notification deadlines. Each new rail follows the established pattern — pure-Python rule under `modules/guardrails/*_rule.py`, thin `@action` wrapper, matrix tests, `rails.co` wiring, `docs/SECURITY.md` row.
