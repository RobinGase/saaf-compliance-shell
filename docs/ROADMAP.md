...
5. **Output-rail matrix expansion.** Ten rails landed by v0.7.0; v0.8.0 tuned four of them (citation, verdict, absolutism, CoT) against adversarial paraphrases surfaced by an independent review. v0.8.2 adds the incident-notification-deadline rail (GDPR Art. 33 72h; NIS2 Art. 23 24h/72h/1mo; DORA Art. 19 4h/72h/1mo). v0.8.3 wires the deadline rail into `output_scan._RAILS` (closing a regression where bypass paths skipped it), rewrites the deadline rule with a trigger-term guard (fires only when notification/reporting language is present in the enclosing sentence), adds multi-framework attribution walking preceding aliases, makes AVG case-sensitive so English "avg" shorthand no longer anchors the rail, and pins the 11-rail registry with a regression test (`tests/test_output_scan.py`). v0.8.4 adds the 12th rail: fabricated case-law / enforcement-action IDs (CJEU `[CTF]-NNN/YY` post-1989, CNIL `SAN-YYYY-NNN` post-2000, plus malformed-separator variants that fire only with CJEU / Court-of-Justice / General Court context in a 120-char window). Candidates still queued: fabricated ENISA/NIS2 incident-report identifiers, fabricated national-DPA enforcement IDs beyond CNIL (Italian Garante, Spanish AEPD, Dutch AP). Each new rail follows the established pattern — pure-Python rule under `modules/guardrails/*_rule.py`, thin `@action` wrapper, matrix tests, `rails.co` wiring, `docs/SECURITY.md` row.

   **Deferred (to v0.8.4):** Commission Delegated Regulation (EU) 2024/1772 Art. 5 appears to set two concurrent initial-notification limits for DORA (4h from classification AND 24h from awareness). Current `_VALID_WINDOWS_HOURS["DORA"]` only contains 4h. Verifying statutory windows from memory is the failure mode the rail exists to catch — this needs OJ verification before the 24h backstop is added.
