...
| Guardrails HTTP wrapper (`:8088`) | Working. PII masking, injection preflight, topical rail. |
...
5. **Branch portability check in CI.** `scripts/check_branch_portability.py` already enforces "no personal hostnames, no Tailscale IPs" on `main` and `modular/*` branches. Wire it into the GitHub Actions workflow.
