...
```
┌──────────────────────────────────────────────────────────────────┐
│  saaf-compliance-shell (single Linux host)                        │
│                                                                   │
│  ┌────────────────────────────────────────────────────────┐      │
...
│  │                   │    │            │                         │
│  │  Input rails:      │    │  Local     │                         │
│  │   PII masking      │───▶│  model     │──localhost──▶ Ollama   │
│  │   (Presidio)       │    │  only (v1) │   (127.0.0.1:8000)     │
│  │   Prompt injection  │    └────────────┘                         │
│  │   Topical control   │                                          │
│  │  Output rails:      │                                          │
│  │   PII masking      │                                          │
...
All inter-module communication is local to a single Linux host. No mTLS in v1 because host services bind to `127.0.0.1`, and the microVM reaches them via the TAP bridge.
...
| NeMo Guardrails (self-check LLM) | local Ollama (direct) | HTTP POST `127.0.0.1:8000/v1/chat/completions` | Guardrails' own intent matching — bypasses router |
| Privacy Router | local Ollama | HTTP POST `127.0.0.1:8000/v1/chat/completions` | OpenAI-compatible API |
...
**Circular dependency resolution:** NeMo Guardrails needs an LLM for its own intent matching (Colang self-check). This connection goes **directly** to the local model endpoint (`127.0.0.1:8000`), not through the Privacy Router. This is safe because guardrails' self-check prompts contain no user data — they are canonical intent matching queries generated by the Colang runtime.
...
The guest agent sees `172.16.0.1:8088` as its inference endpoint. It cannot reach the router (8089), the local model endpoint (`127.0.0.1:8000`), or the internet. All model traffic flows: guest → TAP → guardrails (host) → router (host) → local Ollama.
...
| 1g | Kernel cache | Pre-compiled Amazon Linux microVM kernel (`vmlinux`), cached on the Linux host |
...
host_services:
  ...
  # Router reaches the local model endpoint (127.0.0.1:8000) for inference
...
- The host iptables rules allow the guardrails process to reach both `127.0.0.1:8089` (router) and `127.0.0.1:8000` (direct self-check)
...
models:
  ...
  - type: self_check
    ...
    parameters:
      base_url: http://127.0.0.1:8000/v1  # direct to the local model endpoint for guardrails' own checks
...
```
Model (local host) returns response with CoT
    │
    ▼
...
Firecracker requires Linux with KVM support. In the modular single-host baseline, one Linux machine runs Firecracker, AgentFS, guardrails, router, and the local model endpoint.
...
- The Linux local-host smoke now succeeds with a Python 3.12 venv: `:8088 -> :8089 -> local-model:8000` returns a normal assistant response.
...
| Level | What | Where |
|---|---|---|
| Unit | Presidio recognizers (BSN 11-test, edge cases), Colang flows, policy parser, hash chain logic, manifest validator | local Python environment |
| Integration | Full pipeline: document → VM boot → guardrails → router → local Ollama → guardrails output → audit log | single Linux host |
| Isolation | Firecracker VM boot, TAP networking, iptables enforcement, AgentFS overlay capture | Linux host only (requires KVM) |
| Red team | Adversarial tests (see [Red Team Validation](#red-team-validation)) | Linux host |
...
| Machine | Role | Notes |
|---|---|---|
| **Single Linux host** | Shell host + model host | Must have KVM support and enough CPU, RAM, and disk for Firecracker and Ollama |
| **Optional client machine** | SSH or local terminal access | Used only for development or operations |

The model endpoint stays host-local in the single-host baseline and is never exposed to the public internet in v1.

### Network Flow (single-host)

```
client (optional) ──SSH/local terminal──▶ Linux host
                   │
                   │  saaf-compliance-shell runs here:
                   │    Agent ──▶ Guardrails (127.0.0.1:8088)
                   │              ──▶ Router (127.0.0.1:8089)
                   │                   ──▶ Ollama (127.0.0.1:8000)
                   │              ──▶ Ollama (127.0.0.1:8000)            ← self-check, direct
                   │
...
```bash
# On the Linux host — start the inference endpoint
ollama serve &
ollama pull Randomblock1/nemotron-nano:8b

# Keep the endpoint local to the same host
# Set OLLAMA_HOST=127.0.0.1:8000 in environment
# Ollama serves OpenAI-compatible API at /v1/chat/completions
```

Privacy Router points to: `http://127.0.0.1:8000/v1/chat/completions`
...
```bash
# On the Linux host — build and start vLLM-TurboQuant
...
**The Privacy Router URL does not change between phases** — both Ollama and vLLM serve on the same `127.0.0.1:8000` endpoint with OpenAI-compatible API. Swapping inference backend is transparent to the rest of the stack.

### Localhost Security Considerations

- The model endpoint stays bound to `127.0.0.1:8000` in the single-host baseline.
- The host iptables rules must allow the router and guardrails processes to reach `127.0.0.1:8000`.
- Port 8000 should not be exposed on external interfaces in v1.
- If the local model process goes offline, the entire inference pipeline stops — no automatic failover in v1.

### What Cannot Run This

- **Any low-RAM or no-GPU host** — may not run the local model endpoint acceptably.
- **Any non-Linux host** — cannot host Firecracker directly.
- **Any VM without nested virtualization** — Firecracker needs `/dev/kvm`, so nested virtualization must be enabled.
...
| Direct model access | Agent attempts to reach `127.0.0.1:8000` or any non-guardrails endpoint | iptables drops — guest can only reach `172.16.0.1:8088` (guardrails) |
...
| Attack | Test | Expected Result |
|---|---|---|
| Direct model call | Agent inside VM attempts to reach `127.0.0.1:8000` directly | iptables on TAP drops — guest can only reach `172.16.0.1:8088` (guardrails) |
...
- [x] 3.5 Vendor_Guard integration: add manifest, bake into rootfs, test full pipeline — *end-to-end Vendor_Guard runs complete inside the VM and produce real `scorecard.{xlsx,csv}`, `gap_register.{xlsx,csv}`, and `audit_memo.{docx,html}` dentries under `/audit_workspace/` in the AgentFS overlay. `saaf_entrypoint.log` records `inference_ready=ok` → `pipeline=ok`. Persisting Firecracker stdout/stderr via `console_log_path` was the diagnostic unlock; replacing the per-package NFS `cp` with a prebuilt `/opt/vendor-guard-venv.tar` extraction into tmpfs was the runtime fix.*
...
- `vendor_guard` runs end-to-end inside the VM and produces all final output artifacts (`scorecard.xlsx/csv`, `gap_register.xlsx/csv`, `audit_memo.docx/html`) in the AgentFS overlay.
...
|---|---|---|---|---|
| ~~D1~~ | ~~Primary model host~~ | **RESOLVED** | Single Linux host running `Randomblock1/nemotron-nano:8b` | Phase 1-3: Ollama, Phase 4+: vLLM-TurboQuant |
...
| D4 | Linux host KVM support | Open — **verify in Phase 1** | `lscpu \| grep Virtualization`, `ls /dev/kvm`, `kvm-ok`. If the Linux host is a VM, nested virt must be enabled. | **Blocks entire isolation module** if KVM is unavailable. Fallback: Landlock (weaker but no KVM needed). |
| ~~D5~~ | ~~NIM licensing~~ | **RESOLVED** | Not using NIM container — Ollama (Phase 1-3) then vLLM-TurboQuant (Phase 4+) | No NGC dependency |
| D6 | Guardrails self-check model | Open | Same `Randomblock1/nemotron-nano:8b` on the local host / smaller model via Ollama multi-model | Self-check adds ~2 LLM calls per request. On one host, this competes with user inference. May need a smaller self-check model. |
...
| D9 | Local firewall for port 8000 | Open — **configure in Phase 1** | Keep the model endpoint bound to `127.0.0.1` or restrict external exposure | Prevents accidental exposure of the model endpoint |
