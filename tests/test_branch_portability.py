...
def test_should_enforce_portability_for_main_and_modular() -> None:
    ...
    assert should_enforce_portability("developer/local-sandbox") is False


def test_check_paths_reports_forbidden_machine_specific_strings(tmp_path: Path) -> None:
    target = tmp_path / "config.yml"
    target.write_text("value: FORBIDDEN_PORTABILITY_SENTINEL\n")

    violations = check_paths([target])

    assert len(violations) == 1
    assert "FORBIDDEN_PORTABILITY_SENTINEL" in violations[0]
