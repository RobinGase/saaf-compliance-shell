...
Latest release: **v0.8.0**. Ten output rails, CI-gated branch policy, reproducible release tarball on `dev/main`. v0.8.0 closes the last bypass paths surfaced by an independent review: the salvage path (content recovered from LLM-adapter error strings) now runs the Python output rails before returning; guest isolation extends to IPv6 via `disable_ipv6` on the tap, mirrored ip6tables DROP rules, and a startup gate that refuses to run when either `net.ipv4.ip_forward` or `net.ipv6.conf.all.forwarding` is enabled; `self_check_input`/`self_check_output` refusals join the hash chain as `guardrails_rail_fire` events alongside the ten regex rails. Four rails were also tuned against adversarial paraphrases — the citation rail catches hedged connectives ("under", "in", "as part of"), the verdict rail catches passive-voice "have been fulfilled" and "in full compliance with", the absolutism rail stops false-positing on SLA uptime/availability quotations, and the CoT rail `html.unescape()`s input so HTML-escaped `&lt;think&gt;` still fires. The end-to-end path works: Firecracker + AgentFS + Guardrails + Router + audit log run on a single Linux host, and Vendor_Guard produces real scorecard, gap register, and audit memo artefacts through the VM path. The modular branch is proven enough to support real testing work; it is not yet production-ready. Tracked next steps and open decisions live in [`docs/ROADMAP.md`](docs/ROADMAP.md).
